...
private extension ClassDetailView {
        ...
        GlassCard {
            HStack(alignment: .top, spacing: 12) {
                VStack(alignment: .leading, spacing: 8) {
                    ...
                    Text("\(classModel.grade)\(classModel.letter) • Информация о классе")
    ...
    var summaryStrip: some View {
        HStack(spacing: 12) {
            statCard(title: "Рейтинг класса", value: "\(classModel.classTotalRating)", icon: "star.fill", color: .yellow)
            statCard(title: "Учеников", value: "\(classModel.members.count)", icon: "person.2.fill", color: .green)
        }
    }
        ...
        GlassCard(padding: 16) {
            HStack(spacing: 14) {
                ...
                VStack(alignment: .leading, spacing: 4) {
                    ...
                    Text("\(classModel.teacher.lastName) \(classModel.teacher.name)")
    ...
    var classmatesSection: some View {
        SectionCard(title: "Одноклассники", icon: "person.3.fill") {
            let sortedMembers = classModel.members
            ...
            if sortedMembers.isEmpty {
                EmptyStateView(
                    ...
                    subtitle: "Список учеников пока пуст"
                )
            } else {
                VStack(spacing: 10) {
                    ForEach(Array(sortedMembers.enumerated()), id: \.element.id) { index, user in
                        NavigationLink(destination: UserDetailView(userModel: user)) {
                            LeaderboardRow(
                                ...
                                showChevron: true // Включена стрелка для перехода
                            )
                        }
                        .buttonStyle(.plain)
