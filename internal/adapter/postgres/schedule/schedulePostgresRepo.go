...
import (
	...
	sheduleRepo "cspirt/internal/domain/schedule/repo"
...
func New(db *sql.DB) sheduleRepo.ScheduleRepository {
	return &postgresRepository{db: db}
}

func (r *postgresRepository) GetSchedules(filter scheduleModels.ScheduleFilter) (*scheduleModels.SchedulesResponse, error) {
	...
	load := func(scheduleType string) ([]scheduleModels.ScheduleLesson, error) {
		return r.getScheduleLessonsLocked(scheduleType, filter, 0)
	}
...
func (r *postgresRepository) GetCurrentScheduleForTeacher(teacherID int, filter scheduleModels.ScheduleFilter) ([]scheduleModels.ScheduleLesson, error) {
	if err := r.validateScheduleTeacherLocked(teacherID); err != nil {
		return nil, err
	}
	...
	return r.getScheduleLessonsLocked(scheduleModels.ScheduleTypeCurrent, filter, 0)
}

func (r *postgresRepository) GetScheduleLessonByID(scheduleType string, id int) (*scheduleModels.ScheduleLesson, error) {
	...
	return r.getScheduleLessonByIDLocked(scheduleType, id)
}

func (r *postgresRepository) UpsertScheduleLesson(scheduleType string, lesson scheduleModels.ScheduleLesson) (*scheduleModels.ScheduleLesson, error) {
	scheduleType, err := normalizeScheduleType(scheduleType)
	if err != nil {
		return nil, err
	}
	...
	if err := r.resolveScheduleLessonRefsLocked(scheduleType, &lesson); err != nil {
		return nil, err
	}
	if err := r.validateScheduleLessonRefsLocked(lesson); err != nil {
		return nil, err
	}

	if lesson.ID <= 0 {
		if scheduleType == scheduleModels.ScheduleTypePlanned && lesson.BaseScheduleID != nil {
			existing, err := r.getPlannedScheduleByBaseIDLocked(*lesson.BaseScheduleID)
			if err != nil {
				return nil, err
			}
			if existing != nil {
	...
	if lesson.ID <= 0 {
		existing, err := r.getScheduleLessonByUniqueKeyLocked(scheduleType, lesson.ClassID, lesson.DayOfWeek, lesson.LessonNumber, lesson.WeekType)
		if err != nil {
			return nil, err
		}
		if existing != nil {
	...
	if lesson.ID > 0 {
		if err := r.updateScheduleLessonLocked(scheduleType, lesson); err != nil {
			return nil, err
		}
		return r.getScheduleLessonByIDLocked(scheduleType, lesson.ID)
	}

	id, err := r.insertScheduleLessonLocked(scheduleType, lesson)
	if err != nil {
		return nil, err
	}
...
func (r *postgresRepository) DeleteScheduleLesson(scheduleType string, id int) error {
	scheduleType, err := normalizeScheduleType(scheduleType)
	if err != nil {
		return err
	}
...
func (r *postgresRepository) RolloverSchedules(classID int) (*scheduleModels.ScheduleRolloverResult, error) {
	if classID > 0 {
		exists, err := r.classExistsLocked(classID)
		if err != nil {
			return nil, err
		}
		if !exists {
...
func (r *postgresRepository) ResetPlannedSchedules(classID int) (*scheduleModels.ScheduleResetResult, error) {
	if classID > 0 {
		exists, err := r.classExistsLocked(classID)
		if err != nil {
			return nil, err
...
func (r *postgresRepository) getScheduleLessonsLocked(scheduleType string, filter scheduleModels.ScheduleFilter, id int) ([]scheduleModels.ScheduleLesson, error) {
	table, err := scheduleTable(scheduleType)
	if err != nil {
		return nil, err
	}
...
func (r *postgresRepository) getScheduleLessonByIDLocked(scheduleType string, id int) (*scheduleModels.ScheduleLesson, error) {
	lessons, err := r.getScheduleLessonsLocked(scheduleType, scheduleModels.ScheduleFilter{}, id)
	if err != nil {
		return nil, err
	}
	if len(lessons) == 0 {
...
func (r *postgresRepository) getScheduleLessonByUniqueKeyLocked(scheduleType string, classID int, day string, lessonNumber int, weekType string) (*scheduleModels.ScheduleLesson, error) {
	table, err := scheduleTable(scheduleType)
	if err != nil {
		return nil, err
	}
...
func (r *postgresRepository) getPlannedScheduleByBaseIDLocked(baseScheduleID int) (*scheduleModels.ScheduleLesson, error) {
	if baseScheduleID <= 0 {
		return nil, nil
	}
	...
	return r.getScheduleLessonByIDLocked(scheduleModels.ScheduleTypePlanned, id)
}

func (r *postgresRepository) insertScheduleLessonLocked(scheduleType string, lesson scheduleModels.ScheduleLesson) (int, error) {
	logger.WriteSafe(logger.LogEntry{
		Level:   "info",
...
func (r *postgresRepository) updateScheduleLessonLocked(scheduleType string, lesson scheduleModels.ScheduleLesson) error {
	logger.WriteSafe(logger.LogEntry{
		Level:   "info",
...
func (r *postgresRepository) resolveScheduleLessonRefsLocked(scheduleType string, lesson *scheduleModels.ScheduleLesson) error {
	if scheduleType != scheduleModels.ScheduleTypePlanned || lesson.BaseScheduleID == nil {
		return nil
	}

	base, err := r.getScheduleLessonByIDLocked(scheduleModels.ScheduleTypeBase, *lesson.BaseScheduleID)
	if err != nil {
		return err
	}
	if base == nil {
...
func (r *postgresRepository) validateScheduleLessonRefsLocked(lesson scheduleModels.ScheduleLesson) error {
	if lesson.ClassID <= 0 {
		return errors.New("class id is required")
	}
	exists, err := r.classExistsLocked(lesson.ClassID)
	if err != nil {
		return err
	}
	if !exists {
	...
	return r.validateScheduleTeacherLocked(lesson.TeacherID)
}

func (r *postgresRepository) validateScheduleTeacherLocked(teacherID int) error {
	if teacherID <= 0 {
		return errors.New("teacher id is required")
	}

	var role string
	err := r.db.QueryRow(`SELECT Role FROM users WHERE Id = $1`, teacherID).Scan(&role)
	if err != nil {
		if err == sql.ErrNoRows {
...
func (r *postgresRepository) classExistsLocked(classID int) (bool, error) {
	var exists bool
